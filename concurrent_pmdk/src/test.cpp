...
// Make or Read persistent pool
  ...
  if (file_exists(persistent_path) != 0) {
    pop = pmemobj_create(persistent_path, "btree", 8000000000, 0666); //make 1GB memory pool
        ...
        for(int i = from; i < to; ++i) {
          ...
          switch(jid) {
            case 0:
              D_RW(bt)->btree_insert(keys[i], (char*) keys[i]);
              for(int j = 0; j < 4; j++)
                D_RW(bt)->btree_search(keys[(sidx + j + jid * 8) % half_num_data]);
            ...
            case 1:
              for(int j = 0; j < 3; j++)
                D_RW(bt)->btree_search(keys[(sidx + j + jid * 8) % half_num_data]);
              D_RW(bt)->btree_insert(keys[i], (char*) keys[i]);
              D_RW(bt)->btree_search(keys[(sidx + 3 + jid * 8) % half_num_data]);
              break;
            case 2:
              for(int j = 0; j < 2; j++)
                D_RW(bt)->btree_search(keys[(sidx + j + jid * 8) % half_num_data]);
