...
class page{
  ...
  public:
    ...
    void constructor(PMEMobjpool *pop, page* left, entry_key_t key, page* right, uint32_t level = 0) {
      hdr.constructor();
      for(int i = 0; i<cardinality; i++) {
        records[i].key = LONG_MAX;
        records[i].ptr = NULL;
    ...
    bool remove_rebalancing(btree* bt, entry_key_t key, bool only_rebalance = false, bool with_lock = true) {
      ...
      bt->btree_delete_internal(key, (char *)pmemobj_oid(this).off, hdr.level + 1,
          &deleted_key_from_parent, &is_leftmost_node, (page**)&left_sibling.oid.off);
      ...
      if(is_leftmost_node) {
        ...
        D_RW(hdr.sibling_ptr)->remove(bt, D_RW(hdr.sibling_ptr)->records[0].key, true, with_lock);
      ...
      if(total_num_entries > cardinality - 1) { // Redistribution
        ...
        if(num_entries < left_num_entries) { // left -> right
          if(hdr.leftmost_ptr == nullptr){
            ...
            pmemobj_persist(bt->pop, &(D_RW(left_sibling)->records[m].ptr), sizeof(char *));

            D_RW(left_sibling)->hdr.last_index = m - 1;
            pmemobj_persist(bt->pop, &(D_RW(left_sibling)->hdr.last_index), sizeof(int16_t));

            parent_key = records[0].key; 
          }
          else{
            insert_key(bt->pop, deleted_key_from_parent, (char*)hdr.leftmost_ptr, &num_entries); 
          ...
          if(left_sibling.oid.off == bt->root.oid.off) {
            ...
            bt->btree_insert_internal
              ((char *)left_sibling.oid.off, parent_key, (char *)pmemobj_oid(this).off, hdr.level + 1);
          }
        }
          ...
          if(hdr.leftmost_ptr == nullptr){
            for(int i=0; i<num_dist_entries; i++){
              D_RW(left_sibling)->insert_key(bt->pop, records[i].key, records[i].ptr, &left_num_entries);
            }

            for(int i=num_dist_entries; records[i].ptr != NULL; i++){
              D_RW(new_sibling)->insert_key(bt->pop, records[i].key, records[i].ptr, &new_sibling_cnt, false);
            } 
            ...
            D_RW(left_sibling)->insert_key(bt->pop, deleted_key_from_parent,
            ...
            for(int i=num_dist_entries; records[i].ptr != NULL; i++){
              D_RW(new_sibling)->insert_key(bt->pop, records[i].key, records[i].ptr,
          ...
          if(left_sibling.oid.off == bt->root.oid.off) {
            ...
            bt->btree_insert_internal
              ((char *)left_sibling.oid.off, parent_key, (char *)new_sibling.oid.off, hdr.level + 1);
          }
    ...
    inline void 
      insert_key(PMEMobjpool* pop, entry_key_t key, char* ptr, int *num_entries, bool flush = true,
       ...
       bool flush, bool with_lock, page *invalid_sibling = NULL) {
        ...
        if((hdr.sibling_ptr.oid.off != 0)
          ...
          if(key > D_RO(hdr.sibling_ptr)->records[0].key) {
            ...
            return D_RW(hdr.sibling_ptr)->store(bt, NULL, key, right, 
        ...
        if(num_entries < cardinality - 1) {
          ...
          if(hdr.leftmost_ptr == NULL){ // leaf node
            for(int i=m; i<num_entries; ++i){ 
              sibling_ptr->insert_key(bt->pop, records[i].key, records[i].ptr, &sibling_cnt, false);
            }
          }
          else{ // internal node
